...
public:
    ...
    static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionChangeDate<Traits>>(); }
    ...
    template <class InputDataType, typename ResultDataType>
    ColumnPtr execute(const ColumnsWithTypeAndName & arguments, const DataTypePtr & input_type, const DataTypePtr & result_type, size_t input_rows_count) const
    {
        ...
        for (size_t i = 0; i < result_rows_count; ++i)
        {
            if constexpr (std::is_same_v<InputDataType, DataTypeDateTime64>)
            {
                ...
                result_data[i] = getChangeDate(time, new_value_column_data[i], result_type, date_lut, scale, fraction);
            }
            else if constexpr (std::is_same_v<InputDataType, DataTypeDate32> && std::is_same_v<ResultDataType, DataTypeDateTime64>)
            {
                ...
                result_data[i] = getChangeDate(time, new_value_column_data[i], result_type, date_lut, 3, 0);
            }
            else if constexpr (std::is_same_v<InputDataType, DataTypeDate> && std::is_same_v<ResultDataType, DataTypeDateTime>)
            {
                ...
                Int64 time = static_cast<Int64>(date_lut.toNumYYYYMMDD(input_column_data[i])) * 1'000'000;

                result_data[i] = static_cast<UInt32>(getChangeDate(time, new_value_column_data[i], result_type, date_lut));
            }
            ...
            else
            {
                ...
                if (isDate(result_type))
                    result_data[i] = static_cast<UInt16>(getChangeDate(time, new_value_column_data[i], result_type, date_lut));
                else
                    result_data[i] = static_cast<Int32>(getChangeDate(time, new_value_column_data[i], result_type, date_lut));
            }
        }
    ...
    Int64 getChangeDate(Int64 time, Float64 new_value, const DataTypePtr & result_type, const DateLUTImpl & date_lut, Int64 scale = 0, Int64 fraction = 0) const
    {
        ...
        switch (Traits::component)
        {
            ...
            case Component::Day:
                ...
                break;
            case Component::Hour:
                if (new_value < 1 || new_value > 23)
                    return min_date;
                hours = static_cast<UInt8>(new_value);
                break;
            case Component::Minute:
                if (new_value < 1 || new_value > 59)
                    return min_date;
...
{
    {
        FunctionDocumentation::Description  description = "Change the year component of a date or date time.";
    ...
    {
        FunctionDocumentation::Description  description = "Change the month component of a date or date time.";
    ...
    {
        FunctionDocumentation::Description  description = "Change the day component of a date or date time.";
    ...
    {
        FunctionDocumentation::Description  description = "Change the hour component of a date or date time.";
    ...
    {
        FunctionDocumentation::Description  description = "Change the minute component of a date or date time.";
    ...
    {
        FunctionDocumentation::Description  description = "Change the second component of a date or date time.";
